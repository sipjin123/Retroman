...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class SpeedHackDetector : ActDetectorBase
	{
		...
		private void Update()
		{
			...
			var ticks = GetReliableTicks();
			var ticksSpentSinceLastUpdate = ticks - previousTicks;
			...
			if (ticks - previousIntervalTicks < intervalTicks) return;
			...
			var ticksCheated = Mathf.Abs(vulnerableTicksFromStart - ticksFromStart) > Threshold;

			var vulnerableTime = Time.realtimeSinceStartup;
			var vulnerableTimeFromStart = vulnerableTime - vulnerableTimeOnStart;
			var timeCheated = Math.Abs(ticksFromStart / (float)TicksPerSecond - vulnerableTimeFromStart) > ThresholdFloat;
