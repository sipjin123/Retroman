...
// NOTE: commented out to workaround Unity bug
	...
	public class TimeCheatingDetector : ActDetectorBase
	{
		...
		private void OnSocketConnectedOrSent(object sender, SocketAsyncEventArgs e)
		{
			try
			{
				...
				{
					Debug.LogWarning(LogPrefix + "Socket connected: could not get NTP time from " + timeServer + " =/\n" + e.SocketError);
		...
		private void OnSocketReceive(object sender, SocketAsyncEventArgs e)
		{
			try
			{
				...
				{
					Debug.LogWarning(LogPrefix + "Socket receive: could not get NTP time from " + timeServer + " =/\n" + e.SocketError);
					asyncError = ErrorKind.Unknown;
				}
			}
			catch (Exception exception)
		...
		*/
		...
		public static void StartDetection(float interval, Action detectionCallback = null, Action<ErrorKind> errorCallback = null, Action checkPassedCallback = null)
		{
			Debug.LogError(ErrorMessage);
		}
