...
namespace Sandbox.Audio
{
    ...
    public class DoubleAudioSource : MonoBehaviour
    {
        ...
        public void StopAudio()
        {
            ...
            {
                _source0.Stop();
                //StartCoroutine(fadeSource(_source0, _source0.volume, 0f, 0.2f));
            }
            else
            {
                _source1.Stop();
                //StartCoroutine(fadeSource(_source1, _source1.volume, 0f, 0.2f));
            }
        }
        ...
        IEnumerator Fade(AudioClip playMe, float maxVolume, float fadingTime, float delay_before_crossFade = 0, bool loop = false)
        {
            ...
            _curSourceFadeRoutine = StartCoroutine(fadeSource(curActiveSource, curActiveSource.volume, 0, fadingTime, sameClip));
            _newSourceFadeRoutine = StartCoroutine(fadeSource(newActiveSource, newActiveSource.volume, maxVolume, fadingTime, sameClip));
        ...
        IEnumerator fadeSource(AudioSource sourceToFade, float startVolume, float endVolume, float duration, bool same = false)
        {
            ...
            if (endVolume == 0)
            {
                if (sourceToFade.clip != null && !same)
                    sourceToFade.clip.UnloadAudioData();
            }
        }
