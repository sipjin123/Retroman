...
public class BezierCurve : MonoBehaviour {
    ...
    public void LoadFrom(string json) {
        this.LoadFrom((Dictionary<string, object>)Json.Deserialize(json));
    }

    public void LoadFrom(Dictionary<string, object> shape) {
        /*
        string shapeName = (string)shape["Name"];
        List<object> points = (List<object>)shape["Points"];
        foreach (object obj in points) {
            Dictionary<string, object> point = (Dictionary<string, object>)obj;
            Vector3 localPos = new Vector3()

            // create bezier point
            GameObject pointObject = new GameObject("Point " + this.points.Length);
        ...
        if (curve.pointCount > 0) {
            newPoint.position = curve[curve.pointCount - 1].position;
        }

        newPoint.curve = curve;
        //newPoint.handle1 = Vector3.right*0.1f;
        //newPoint.handle2 = -Vector3.right*0.1f;
        newPoint.handle1 = Vector3.zero;
        newPoint.handle2 = Vector3.zero;
        //*/
    }
    ...
    /// <summary>
	/// 	- Clears all the points in Bezier
	/// </summary>
    public void ClearPoints() {
        List<BezierPoint> tempArray = new List<BezierPoint>(points);
        while (tempArray.Count > 0) {
            BezierPoint point = tempArray[0];
    ...
    public BezierPoint[] GetInBetweenAnchors(float t) {
        if (t <= 0 || t >= 1) {
            return new BezierPoint[0];
        }
        ...
        return new BezierPoint[2] { p1, p2 };
    }
    ...
    public Vector3 GetPointAt(float t)
	{
        ...
        //return GetPoint(p1, p2, t / curvePercent).ToLocal(this.transform);
    }
    ...
    public static float ApproximateLengthWithPreviousCounted(BezierCurve curve, BezierPoint p1, BezierPoint p2, int resolution = 30) 
    {
        BezierPoint[] points = curve.GetAnchorPoints();
        int finalIndex = curve.GetPointIndex(p1);
        float unitPerc = 0;

        if (finalIndex > 0) {
            for (int i = 0; i <= finalIndex - 1; i++) {
                unitPerc += ApproximateLength(points[i], points[i + 1], resolution);
            }
        }

        unitPerc += ApproximateLength(p1, p2, resolution);
        return unitPerc;
    }
	...
	public Vector3 GetPointAtDistance(float distance)
	{
        ...
        //return GetPoint(firstPoint, secondPoint, distance / curveLength).ToLocal(this.transform);
    }
	//*/
}