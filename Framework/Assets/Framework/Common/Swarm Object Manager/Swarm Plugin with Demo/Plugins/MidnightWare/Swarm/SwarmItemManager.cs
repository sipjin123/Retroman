...
/// </summary>
public class SwarmItemManager : Behaviour
{
    /// <summary>
    ...
    public virtual SwarmItem ActivateItem(int itemPrefabIndex, Transform parent)
    {
        return ActivateItem(itemPrefabIndex, parent, SwarmItem.STATE.Active);
    }

    public virtual SwarmItem ActivateItem(int itemPrefabIndex, Transform parent, SwarmItem.STATE state)
    {
        // we have exceeded the maximum item count for this prefab (if a limit is set)
        // so return nothing
        if (_prefabItemLists[itemPrefabIndex].activeItems.Count == itemPrefabs[itemPrefabIndex].maxItemCount && itemPrefabs[itemPrefabIndex].maxItemCount > 0)
        ...
        // move the item to active parent transform.
        // this is mainly just for visual reference in the editor
        SetItemParentTransform(localItem, parent);

        // set the state to custom
        localItem.State = SwarmItem.STATE.Active;
    ...
    protected virtual SwarmItem InstantiateItem(int itemPrefabIndex)
    {
        ...
        //_go.name = "[" + _itemCount.ToString("0000") + "] " + _go.name.Replace("(Clone)", _go.activeSelf.To01());
        _go.name = "[" + _itemCount.ToString("0000") + "] " + _go.name.Replace("(Clone)", string.Empty);
    ...
    {
        int prefabItemLength = _prefabItemLists.Length;

        // iterate through each SwarmItem type
        for (int i = 0; i < prefabItemLength; i++)
        {
    ...
    public virtual void PruneInactiveList(int itemPrefabIndex, int maxCount)
    {

    }

    public virtual void KillAllActiveItems()
    {

    }

    public virtual void Preload(int itemPrefabIndex, int preloadCount)
    {
